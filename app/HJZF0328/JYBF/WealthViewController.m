...
-(void)makeUI
{
    ...
    //余额
    UIImageView *yu_eImage=[[UIImageView alloc] initWithFrame:CGRectMake(15, 15, 50*screenWidth/320.0, 50*screenWidth/320.0)];
    yu_eImage.image=[UIImage imageNamed:@"yuezuo@2x.png"];
    [view addSubview:yu_eImage];
    
    UILabel *Ylabel=[[UILabel alloc] initWithFrame:CGRectMake((view.frame.size.width/2-90)/(screenWidth/320.0), 15, 60*screenWidth/320.0, 30*screenWidth/320.0)];
    Ylabel.text=@"余额";
    Ylabel.textAlignment=NSTextAlignmentCenter;
    Ylabel.textColor=[UIColor whiteColor];
    Ylabel.font=[UIFont systemFontOfSize:16*screenWidth/320.0];
    ...
    int a = [user.yu intValue];
    user.yu=[NSString stringWithFormat:@"%d",a];
    user.KYyue=[dic objectForKey:@"avlBal"];
    NSLog(@"#################yu e%@",dic);
    
    YEtext=[[UILabel alloc] initWithFrame:CGRectMake((view.frame.size.width/2-100)/(screenWidth/320.0), 50, 85*screenWidth/320.0, 30*screenWidth/320.0)];
    YEtext.textColor=[UIColor whiteColor];
    YEtext.font=[UIFont systemFontOfSize:14*screenWidth/320.0];
    YEtext.textAlignment=NSTextAlignmentCenter;
    YEtext.text=[NSString stringWithFormat:@"¥%@元",user.yu];
    [view addSubview:YEtext];
    
    UIButton *yu_eButton=[UIButton buttonWithType:UIButtonTypeCustom];
    yu_eButton.frame=CGRectMake(0, 0, self.view.frame.size.width/2-0.2, view.frame.size.height);
    [yu_eButton addTarget:self action:@selector(yu_eBtn) forControlEvents:UIControlEventTouchUpInside];
    yu_eButton.backgroundColor=[UIColor clearColor];
    [view addSubview:yu_eButton];
    //
    UIView *view1=[[UIView alloc] initWithFrame:CGRectMake(view.frame.size.width/2.0+0.2, 0, 0.4, view.frame.size.height)];
    view1.backgroundColor=[UIColor lightGrayColor];
    [view addSubview:view1];
    //
    //
    //返佣
    UIImageView *FYimage=[[UIImageView alloc] initWithFrame:CGRectMake(self.view.frame.size.width/2+16, 15, 50*screenWidth/320.0, 50*screenWidth/320.0)];
    FYimage.image=[UIImage imageNamed:@"fanyongyou@2x.png"];
    [view addSubview:FYimage];
    
    UILabel *FYlabel=[[UILabel alloc] initWithFrame:CGRectMake(240*screenWidth/320.0, 15, 60*screenWidth/320.0, 30*screenWidth/320.0)];
    FYlabel.text=@"返佣";
    FYlabel.textAlignment=NSTextAlignmentCenter;
    FYlabel.textColor=[UIColor whiteColor];
    FYlabel.font=[UIFont systemFontOfSize:16*screenWidth/320.0];
    [view addSubview:FYlabel];
    ...
    int b=[user.fanYong intValue];
    user.fanYong=[NSString stringWithFormat:@"%d",b];
    NSLog(@"#################fan yong%@",dic);
    
    FYtext=[[UILabel alloc] initWithFrame:CGRectMake((240-13)*screenWidth/320.0, 50, 85*screenWidth/320.0, 30*screenWidth/320.0)];
    FYtext.textColor=[UIColor whiteColor];
    FYtext.font=[UIFont systemFontOfSize:14*screenWidth/320.0];
    FYtext.textAlignment=NSTextAlignmentCenter;
    FYtext.text=[NSString stringWithFormat:@"¥%@元",user.fanYong];
    [view addSubview:FYtext];
    
    NSMutableURLRequest *rever3 = [PostAsynClass postAsynWithURL:url1 andInterface:url13 andKeyArr:@[@"merId",@"acctType"]andValueArr:@[user.merid,@"JF00"]];
    //开始post请求
    NSData *receive3 = [NSURLConnection sendSynchronousRequest:rever3 returningResponse:nil error:nil];
       NSString  *str3 = [[NSString alloc] initWithData:receive3 encoding:enc];
    ...
    user.jiFen=[dic3 objectForKey:@"acctBal"];
    NSLog(@"#################ji fen%@",dic3);
    
    UIButton *FYbutton=[UIButton buttonWithType:UIButtonTypeCustom];
    FYbutton.frame=CGRectMake(self.view.frame.size.width/2+0.2, 0, self.view.frame.size.width/2, view.frame.size.height);
    [FYbutton addTarget:self action:@selector(fyBtn) forControlEvents:UIControlEventTouchUpInside];
    FYbutton.backgroundColor=[UIColor clearColor];
    [view addSubview:FYbutton];
    
    
    int m=0;
    int n=0;
    NSArray*nineImageArray=@[@"WX1",@"WX0",@"",@"",
                             @"ZZ",@"",@"",@"JRCS",
                             @"SC",@"",@"",@"",
                             @"",@"",@"",@"LLB"];
    NSArray *nineTitileArray=@[@"微信T1",@"微信T0",@"Apple Pay",@"",
                               @"转账",@"一元许愿",@"卡务管理",@"金融超市",
                               @"商城",@"网上小贷",@"信用卡提额",@"信用卡申请",
                               @"信用卡还款",@"圈里圈外",@"分润贷款",@"4G流量"];
...
-(void)allBtnClick:(UIButton *)allBtn
{
    ...
    switch (allBtn.tag)
    {
        ...
        case 303:
        {
            customViewController *vc = self.navigationController.viewControllers[1];
            [((MyTabbar *)[vc.view viewWithTag:2016]) selectWithIndex:1];
            vc.selectedIndex = 1;
