//
//  weixinErweimaViewController.m
//  JYBF
//
//  Created by 王健超 on 16/1/6.
...
#import "WXApi.h"
@interface weixinErweimaViewController ()
{
    NSString *shareUrl;
}
@end

@implementation weixinErweimaViewController
...
- (void)viewDidLoad {
    ...
    shareUrl=[NSString stringWithFormat:@"%@%@?merId=%@&liqType=%@",url1,url40,user.merid,self.type];
    //网址字符串
    NSMutableURLRequest *rever = [PostAsynClass postAsynWithURL:url1 andInterface:url40 andKeyArr:@[@"merId",@"liqType"]andValueArr:@[user.merid,self.type]];
...
-(void)navButtonClick:(UIButton *)btn
{
    ...
    id<ISSShareActionSheetItem> item1 = [ShareSDK shareActionSheetItemWithTitle:@"微信" icon:[UIImage imageNamed:@"lh_weixin1.png"] clickHandler:^{
        id<ISSContent> publishContent = [ShareSDK content:[NSString stringWithFormat:@"来自%@的分享%@",user.name,shareUrl] defaultContent:@"默认分享内容，没内容时显示" image:[ShareSDK imageWithPath:imagePath] title:@"企盟支付" url:shareUrl description:@"企盟支付收款分享" mediaType:SSPublishContentMediaTypeNews];
    ...
    id<ISSShareActionSheetItem> item2 = [ShareSDK shareActionSheetItemWithTitle:@"短信" icon:[UIImage imageNamed:@"lh_duanxin.png"] clickHandler:^{
        id<ISSContent> publishContent = [ShareSDK content:@"企盟支付" defaultContent:@"默认分享内容，没内容时显示" image:[ShareSDK imageWithPath:imagePath] title:@"企盟支付" url:shareUrl description:@"企盟支付收款分享" mediaType:SSPublishContentMediaTypeNews];
        //设置短信内容
        [publishContent addSMSUnitWithContent:[NSString stringWithFormat:@"来自%@的分享企盟支付%@",user.name,shareUrl]];
    ...
    id<ISSShareActionSheetItem> item3 = [ShareSDK shareActionSheetItemWithTitle:@"朋友圈" icon:[UIImage imageNamed:@"lh_pengyouquan.png"] clickHandler:^{
        id<ISSContent> publishContent = [ShareSDK content:[NSString stringWithFormat:@"来自%@的分享 企盟支付%@",user.name,shareUrl] defaultContent:@"默认分享内容，没内容时显示" image:[ShareSDK imageWithPath:imagePath] title:@"企盟支付" url:shareUrl description:@"企盟支付收款分享" mediaType:SSPublishContentMediaTypeNews];
