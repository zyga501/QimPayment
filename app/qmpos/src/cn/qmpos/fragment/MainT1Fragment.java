...
public class MainT1Fragment extends Fragment implements OnClickListener {
	...
	private View weixint1_layout, weixint0_layout, jifen_layout,
			zhuanzhang_layout, yiyuanduobao_layout, xinyongkagl_layout,
			liuliang_layout, jrchaoshi_layout, shangcheng_layout;
	...
	private void init(View t1Layout) {
		...
		String pay0Amt = sp.getString("PAY0_avlBal", "0.00");
		String rateAmt = sp.getString("RATE_avlBal", "0.00");
		String acctBal = sp.getString("PAY0_acctBal", "0.00");

		String jfAmt = CommUtil.removeDecimal(sp.getString("JF00_avlBal",
				"0.00"));
		...
		yiyuanduobao_layout = t1Layout.findViewById(R.id.yiyuanduobao_layout);
		xinyongkagl_layout = t1Layout.findViewById(R.id.xinyongkagl_layout);
		...
		yiyuanduobao_layout.setOnClickListener(this);
		xinyongkagl_layout.setOnClickListener(this);
	...
	public void onClick(View v) {
		try {
			...
			case R.id.weixint1_layout:
				...
				url = Constants.server_host + Constants.server_doRecvQrCode_url
						+ "?merId=" + merId + "&liqType=T1";
			...
			case R.id.zhuanzhang_layout:
				...
				break;
			// 一元夺宝
			case R.id.yiyuanduobao_layout:
				Toast.makeText(mainActivity, "暂未开通", Toast.LENGTH_SHORT).show();
				break;
			// 信用卡攻略
			case R.id.xinyongkagl_layout:
				Toast.makeText(mainActivity, "暂未开通", Toast.LENGTH_SHORT).show();
				break;
			// 流量包购买
			case R.id.liuliang_layout:
				intent = new Intent(mainActivity, WebViewActivity.class);
				url = Constants.server_host + Constants.server_extSysLogin_url
						+ "?agentId=" + Constants.server_agent_id + "&merId="
						+ merId + "&extSysId=" + "0001";
				intent.putExtra("url", url);
				intent.putExtra("title", "流量包购买");
				startActivity(intent);
				break;
			// 金融超市
	...
	class InitTask extends AsyncTask<String, Integer, HashMap<String, String>> {
		...
		protected HashMap<String, String> doInBackground(String... params) {
			...
			try {
				...
				returnMap.put("debitFeeRateT0",
						jsonObj.getString("debitFeeRateT0"));
				returnMap.put("debitFeeRateT1",
						jsonObj.getString("debitFeeRateT1"));
