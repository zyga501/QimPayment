...
@interface fanYongJiLuViewController ()
{
...
@implementation fanYongJiLuViewController
...
-(void)makeNav
{
    ...
    [mnb createMyNavigationBarWithBgImageName:nil andLeftBBIIamgeNames:[NSArray arrayWithObject:@"title_back.png"] andRightBBIImages:nil andTitle:@"返佣记录" andClass:self andSEL:@selector(bacClick:)];
...
-(void)LoadData
{
    ...
    NSMutableURLRequest *rever =[PostAsynClass postAsynWithURL:url1 andInterface:url24 andKeyArr:@[@"merId",@"beginDate",@"endDate",@"pageNum",@"pageSize"]andValueArr:@[user.merid,_timeStr,_timeStr1,[NSString stringWithFormat:@"%d",_pageNum],@"12"]];
    //
    ...
    if(_dataArr.count<1)
    {
        UIAlertView *alat = [[UIAlertView alloc] initWithTitle:@"提示" message:@"近期内你还没有返佣交易" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
